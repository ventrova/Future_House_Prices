...
sales_price <- trainX$SalePrice
train_num <- train_num[,1:37]

#MSSubclass Categorical Predictor
...
datPart <- createDataPartition(sales_price, p=.7, list = FALSE)
...
sales_priceTrain= sales_price[datPart,]

# regression models -------------------------------------------------------


#ordinary linear regression only numerical predictors
lm_num <- lm(num ~., data = numTrain )
summary(lm_num)
