...
svmGrid <-expand.grid(.C = c(.25, .5, 1, 1.5, 2, 4, 8),
                      .sigma = c(.001, .01, .1))

svmTune <- train(binTrain[,1:b],
                 binTrain$sales_price,
                 method = "svmRadial",
                 tuneGrid = svmGrid,
                 trControl = ctrl)
svmTune

svmFit <- ksvm(sales_price ~., data = binTrain,
               scaled = FALSE, C = 8, epsilon = .001)

svmFit

svmPred <- predict(svmFit, binTest[,1:b])
defaultSummary(data.frame(obs = binTest$sales_price, pred =svmPred))
plot(binTest$sales_price ~ svmPred, 
     xlab = "predicted",
     ylab = "observed",
     main = "SVM")
# xlim = c(-5,5))
...
write.csv(unscale(Kagglelm_pcap), file = "PCA_Predict")
...
#svm 
svmFit <- ksvm(sales_price ~., data = binKaggleT,
               scaled = FALSE, C = 8, epsilon = .001)

svmPred <- predict(svmFit, testSetT)
write.csv(rfPred, file = "svmPredict")
